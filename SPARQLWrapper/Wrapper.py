...
# Possible HTTP Authentication methods - DEPRECATED
BASIC = "BASIC"
...
class SPARQLWrapper(object):
    ...
    Attributes:
        endpoint: str -> SPARQL endpoint's URI.
        updateEndpoint: str -> SPARQL endpoint's URI for SPARQL Update operations (if it's a different one).
            **Default** is ``None``.
        agent: str -> User-Agent for HTTP request header. **Default** value is an autogenerated string using
            SPARQLWrapper version code.
        _defaultGraph: str -> URI for the default graph. Value can be set either via explicit call
            :func:`addParameter("default-graph-uri", uri)<addParameter>` or as part of the query string.
            **Default** value is ``None``.
        onlyConneg: bool -> Option for allowing (or not) **only** HTTP Content Negotiation (so dismiss the use of HTTP
            parameters). **Default** value is ``False``.
        customHttpHeaders: dict -> Custom HTTP Headers to be included in the request. A dictionary where keys are header
            field and values are header values. **Important**: These headers override previous values (including
            ``Content-Type``, ``User-Agent`` and ``Accept`` if present).
        timeout: int -> Timeout (in seconds) to use for endpoint querying.
        queryString: str -> The SPARQL query text.
        queryType: str -> Type of SPARQL query (aka SPARQL query form), like :data:`CONSTRUCT`, :data:`SELECT`,
        ...
        returnFormat: str -> The return format. **Important:** No local check is done, parameter is simply sent to
            endpoint. Eg, if the value is set to :data:`JSON` and a construct query is issued, it is up to the endpoint
            to react or not. Possible values are :data:`JSON`, :data:`XML`, :data:`TURTLE`, :data:`N3`, :data:`RDF`,
            :data:`RDFXML`, :data:`CSV`, :data:`TSV`, :data:`JSONLD` (constants in this module).
            The **default** value is :data:`XML`.
        requestMethod: str -> The request method for query or update operations. The possibles values are URL-encoded
            (:data:`URLENCODED`) or POST directly (:data:`POSTDIRECTLY`).
        method: str -> Invocation method (HTTP verb).**Default** value is :data:`GET`, but can be set to :data:`POST`.
        parameters: dict -> The parameters of the request (key/value pairs in a dictionary).
        _defaultReturnFormat: str -> Default return format. Used in case the same class instance is reused for
            subsequent queries
        session: requests.Session -> Expects already authorized session, if authorization is needed, otherwise a new one
            is generated without any authentication. **Default** is ``None``.
        prefix_pattern: class -> Regular expression used to remove base/prefixes in process of query
            type determination. :class:`re.RegexObject`, a compiled regular expression. See the :mod:`re` module
            of Python.
        pattern: class -> Regular expression used to determine whether a query (without base/prefixes)
            is of type :data:`CONSTRUCT`, :data:`SELECT`, :data:`ASK`, :data:`DESCRIBE`, :data:`INSERT`, :data:`DELETE`,
            :data:`CREATE`, :data:`CLEAR`, :data:`DROP`, :data:`LOAD`, :data:`COPY`, :data:`MOVE` or :data:`ADD`.
            :class:`re.RegexObject`, a compiled regular expression. See the :mod:`re` module of Python.
        comments_pattern: class -> regular expression used to remove comments from a query.
            :class:`re.RegexObject`, a compiled regular expression. See the :mod:`re` module of Python.
    ...
    def __init__(
        ...
        **default** value is ``None``.
        ...
        :param agent: The User-Agent for the HTTP request header. **Default** value is an autogenerated string
        using the SPARQLWrapper version number.
        :type agent: string
        :param session: The Session for the endpoint authentication. **Default** value is a new Session without
        any Authentication values
        :type session: requests.Session
        """
        self.endpoint = endpoint
        self.updateEndpoint = updateEndpoint or endpoint
        self.agent = agent
    ...
    def setReturnFormat(self, format: str) -> None:
        ...
        if format in _allowedFormats:
            ...
            warnings.warn(
                f"""Ignore format '{format}'; current instance supports: {", ".join(_allowedFormats)}.""",
                SyntaxWarning,
            )
    ...
    def setRequestMethod(self, method: str) -> None:
        ...
        if method in _REQUEST_METHODS:
            ...
            warnings.warn(f"invalid update method '{method}'", RuntimeWarning)

    @deprecation.deprecated(deprecated_in="1.6.0", current_version=__version__,
                            details="Use addParameter('default-graph-uri', uri)<addParameter>` instead")
    def addDefaultGraph(self, uri: str) -> None:
        """
        Add a default graph URI.

        **DEPRECATED**: In version 1.6.0 - Use :func:`addParameter("default-graph-uri", uri)<addParameter>`
    ...
    @deprecation.deprecated(deprecated_in="1.6.0", current_version=__version__,
                            details="Use addParameter('named-graph-uri', uri) instead")
    def addNamedGraph(self, uri: str) -> None:
        """
        Add a named graph URI.

        **DEPRECATED**: In version 1.6.0 - Use :func:`addParameter("named-graph-uri", uri)<addParameter>`
            instead of this method.
    ...
    @deprecation.deprecated(deprecated_in="1.6.0", current_version=__version__,
                            details="Use addParameter(key, value) instead")
    def addExtraURITag(self, key: str, value: str) -> None:
        ...
        **DEPRECATED**: In version 1.6.0 - Use :func:`addParameter(key, value)<addParameter>` instead of this method
    ...
    def addCustomParameter(self, name: str, value: str) -> bool:
        ...
        **DEPRECATED**: In version deprecated:: 1.6.0 - Use :func:`addParameter(key, value)<addParameter>`
            instead of this method
    ...
    @deprecation.deprecated(deprecated_in="2.0.0", current_version=__version__,
                            details="Use requests.Session instead")
    def addCustomHttpHeader(self, httpHeaderName: str, httpHeaderValue: str) -> None:
        ...
        **DEPRECATED**: In version deprecated: 2.0.0 - Use a prepared requests.Session instead of this method
    ...
    def _parseQueryType(self, query: str) -> Optional[str]:
        ...
        warnings.warn(f"unknown query type '{r_queryType}'", RuntimeWarning)
        return SELECT
    ...
    def setUseKeepAlive(self) -> None:
        ...
        **DEPRECATED:: In version 2.0.0. - Use requests.Session instead
    ...
    def _getAcceptHeader(self) -> str:
        ...
        if self.queryType in [SELECT, ASK]:
            if self.returnFormat == XML:
                ...
                warnings.warn(
                    f"Sending Accept header '*/*' because unexpected returned format '{self.returnFormat}' in a '{self.queryType}' SPARQL query form",
                    RuntimeWarning,
                )
            ...
            elif self.returnFormat in [XML, RDFXML]:
    ...
    def _query(self) -> tuple[requests.models.Response, str]:
        """Internal method to execute the query. Returns the output of the
        :func:`requests.Session` method of the :mod:`Requests` Python library.
        The session object is passed by the user or created without any authentication.
        """
    ...
    def query(self) -> "QueryResult":
        ...
        The usual requests exceptions are raised, which therefore cover possible SPARQL errors, too.
    ...
    def __str__(self) -> str:
        ...
        :rtype: string
        """
        fullname = f"{self.__module__}.{self.__class__.__name__}"
...
class QueryResult(object):
    """
    Wrapper around query result. Users should not create instances of this class, it is
    ...
    * ``info()`` that returns the meta-information of the HTTP result as a dictionary-like object.
    ...
    means that the common idiom ``for l in obj : do_something_with_line(l)`` would work, too. Note: DEPRECATED
    ...
    def __init__(self, result: Union[requests.models.Response, Tuple[requests.models.Response, str]]) -> None:
        """
    ...
    @deprecation.deprecated(deprecated_in="2.0.0", current_version=__version__,
                            details="requests does not need another __iter__ method")
    def __iter__(self) -> Iterator[bytes]:
        """Return an iterator object. This method is expected for the inclusion
        of the object in a standard ``for`` loop.

        **DEPRECATED:: In version 2.0.0. - It is possible to iterate through requests objects. Method not needed.
        """
        return self.response.__iter__()

    @deprecation.deprecated(deprecated_in="2.0.0", current_version=__version__,
                            details="requests does not need another __next__ method")
    ...
    def convert(self) -> ConvertResult:
        ...
        def _content_type_in_list(real: str, expected: List[str]) -> bool:
            ...
            :rtype: boolean
            """
            return True in [mime in real for mime in expected]
        ...
        if "content-type" in self.info():
            ...
            if _content_type_in_list(ct, _SPARQL_XML):
                ...
                warnings.warn(
                    f"unknown response content type '{ct}' returning raw response...",
                    RuntimeWarning,
                )
        return self.response.content
    ...
    def _get_responseFormat(self) -> Optional[str]:
        ...
        if "content-type" in self.info():
            ...
            if _content_type_in_list(ct, _SPARQL_XML):
                ...
                warnings.warn(
                    f"Unknown response content type. Returning raw content-type ('{ct}').",
                    RuntimeWarning,
                )
                return ct
        return None
    ...
    def print_results(self, minWidth: Optional[int] = None) -> None:
        ...
        response_format = self._get_responseFormat()
        if response_format != JSON:
            message = "Format return was %s, but JSON was expected. No printing."
            warnings.warn(message % response_format, RuntimeWarning)
            return None
        ...
        for index, var in enumerate(results["head"]["vars"]):
            print(f"?{var}".ljust(width[index]), "|")
        print()
    ...
    def __get_results_width(
            self, results: Dict[Any, Any], min_width: int = 2
    ) -> List[int]:
        width: List[int] = [
            max(min_width, len(var) + 1) for var in results["head"]["vars"]
        ]
        for result in results["results"]["bindings"]:
            for index, var in enumerate(results["head"]["vars"]):
                result_value = self.__get_prettyprint_string_sparql_var_result(
    ...
    def __str__(self) -> str:
        ...
        """
        fullname = f"{self.__module__}.{self.__class__.__name__}"
        str_requested_format = f'"requestedFormat" : {repr(self.requestedFormat)}'
        str_url = self.response.url
        str_code = self.response.status_code
        str_headers = self.response.headers
        str_response = (
